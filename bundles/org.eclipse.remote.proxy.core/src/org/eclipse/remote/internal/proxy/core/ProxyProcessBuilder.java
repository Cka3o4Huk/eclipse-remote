...
public class ProxyProcessBuilder extends AbstractRemoteProcessBuilder {
	...
	public IRemoteProcess start(int flags) throws IOException {
		...
		final MultiplexedChannel chanStdIO = conn.openChannel();
		final MultiplexedChannel chanStdErr = conn.openChannel();
		final MultiplexedChannel chanControl = conn.openChannel();
		...
		// TODO start remote process
		return new ProxyProcess(getRemoteConnection(), this, 
				chanStdIO.getInputStream(), 
				chanStdErr.getInputStream(), 
				chanStdIO.getOutputStream(), 
				chanControl.getInputStream(),
				chanControl.getOutputStream());
	}
}
